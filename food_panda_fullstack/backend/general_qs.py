...
def handle_general_query(initial_messages: list[tuple[str, str]], user_message: str) -> tuple[str, bool]:
    ...
    client = get_chroma_client(collection_name="foodpanda_rag_kb")
    
    releted_docs = client.similarity_search_with_score(query=user_message, k=3)
    context_text = "\n\n---\n\n".join(
    ...
    agent = create_agent(model, tools = tools, system_prompt=f"You are a food panda support agent. your task is to answer user questions based on the knowledge base and if you do not know the answer say sorry and do not make up information from your own. Make sure that if you think that the user have more questions to ask in this conversation send the is_finished flag as False.\n\n Context: {context_text}", response_format=GeneralChatOutput)
